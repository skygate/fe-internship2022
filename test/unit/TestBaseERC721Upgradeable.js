...
describe("Test base ERC721 Upgradeable", function () {
    ...
    it("TEST upgrade - PASS", async () => {
        const sendEthAmount = ethers.utils.parseEther('0.05');
        const tokenId = 0;
        const mintTx = await baseERC721Upgradeable.connect(addr1).payToMint(
            addr1.address, metadataURI, { value: sendEthAmount }
        );
        await mintTx.wait();

        await expect(baseERC721Upgradeable.connect(addr2).burn(tokenId))
            .to.be.revertedWith("Cant perform this action, you must be owner of this token!");

        const BaseERC721UpgradeableV2 = await ethers.getContractFactory("BaseERC721UpgradeableV2");
        const baseERC721UpgradeableV2 = await upgrades.upgradeProxy(baseERC721Upgradeable, BaseERC721UpgradeableV2);

        await expect(baseERC721Upgradeable.connect(addr2).burn(tokenId))
            .to.be.revertedWith("BaseERC721 Error: Can't perform this action, you must be owner of this token!");

        await expect(baseERC721Upgradeable.connect(addr1).startSale(tokenId, 0))
